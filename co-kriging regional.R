...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#first need to map states in the data file to the regions and create an aggregate shapefile outline.
ng_lga <- st_read("/Users/matthewnicholson/Downloads/nigeria-lgas/new_lga_nigeria_2003.shp")
shp_states <- unique(ng_lga$STATE) |> 
  trimws()
BRdata <- BRdata |> 
  mutate(sstate = as_factor(sstate)) 


data_states <- c("Akwa Ibom", "Anambra", "Benue", "Cross River", "Imo", "Rivers")

ng_lga <- ng_lga |> 
  dplyr::filter(STATE %in% data_states) |> 
  st_as_sf() |> 
  st_union() |> st_make_valid()
plot(ng_lga) #oops! wrong region?
# unique(ng_lga$STATE)
