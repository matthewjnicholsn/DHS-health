...
suppressWarnings({
  ...
  for (i in seq_along(file_paths)) {
    ...
    #this makes the whole script work? but it messes with coordinate hence theme_void in the plots 
    #this is an obvious problem because I want to make maps later on that include multiple
    #countries
    sf_use_s2(FALSE)
    
    #get centroids for labels in plot (fake centroids)
    aggregate <- aggregate %>% 
   ...
   p <- ggplot(data = aggregate) +
     ...
     scale_fill_gradient(low = "red", high = "yellow", limits = c(1, 5), oob = scales::squish) +  # Fixed limits
