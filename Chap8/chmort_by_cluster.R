...
file_paths <- get_file("Kenya_DHS", 2003, "BR")
...
BRdata_CMORT <- (BRdata[, c("v021", "v022","v024", "v025", "v005", "v008","v011", 
                            ...
                            "birth_order", "prev_bint","birth_size", clust_var)]) #account for deprecated naming conventions
...
########################################################################################
#write chmort tables by cluster (only works 2003 onwards, different var naming)

# Proceed with the logic that requires clust_var
if (!is.null(clust_var)) {
  # Ensure that clust_var is a valid column name in BRdata
  if (clust_var %in% names(BRdata)) {
    BRdata <- BRdata %>%
      mutate(cluster = as_factor(get(clust_var)))  # Use get() to dynamically reference the variable
  } else {
    warning(paste("Column", clust_var, "does not exist in BRdata."))
  }
  
  # Check cluster is factor
  BRdata_CMORT_cluster <- BRdata %>% 
    select(v021, v022, v024, v025, v005, v008, v011, # Subset
           b3, b7, v106, v190, child_sex, mo_age_at_birth, 
           birth_order, prev_bint, birth_size, clust_var) %>%
    filter(!is.na(cluster) & !is.na(v021) & !is.na(v022) & !is.na(v024) & !is.na(v025)) # Remove NA
  
  res_cluster <- as.data.frame(chmort(BRdata_CMORT_cluster, Class = "cluster", Period = 120)) # Create chmort table by cluster
  
  write.xlsx(res_cluster, paste0("Tables_child_mort_by_cluster", 0 + i, ".xlsx"), asTable = TRUE, append = TRUE, overwrite = FALSE) # Write to Excel
}
else {
  # If clust_var is NULL, skip to the next iteration
  warning("clust_var is NULL. Skipping iteration.")
