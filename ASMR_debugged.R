ASMR <-function (Data.Name, CL = NULL, PeriodEnd = NULL, Period = NULL) 
{
  ...
  if (is.null(PeriodEnd)) {
    PeriodEndy_ <- as.integer((mean(Data.Name$v008) - 1)/12) + 
      1900
    PeriodEndm_ <- round(mean(Data.Name$v008) - ((PeriodEndy_ - 
                                                    1900) * 12), 0)
    PeriodEndm_m <- round(min(Data.Name$v008) - ((PeriodEndy_ - 
                                                    1900) * 12), 0)
    PeriodEndm_x <- round(max(Data.Name$v008) - ((PeriodEndy_ - 
                                                    1900) * 12), 0)
  }
  else {
    ...
    if (PeriodEndm_ >= round(mean(Data.Name$v008) - (((as.integer((mean(Data.Name$v008) - 
  ...
  # cat("NAs in v021:", sum(is.na(DeathEx$v021)), "\n")
  # cat("NAs in v022:", sum(is.na(DeathEx$v022)), "\n")
  # cat("NAs in rweight:", sum(is.na(DeathEx$rweight)), "\n")
  # cat("Rows with any NA in these:", sum(!complete.cases(DeathEx[,c("v021","v022","rweight")])), "\n")
