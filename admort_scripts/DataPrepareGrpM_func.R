...
DataPrepareGrpM <- function (Dat, PeriodEnd = NULL, Period = NULL, group_var = NULL) 
{
  ...
  myvarsid <- c("ID", "v021", "v005", "v008", "v022", "rweight", group_var) 
 
  new_list <- list()  # Initialize an empty list to store the modified variables
  
  for (i in seq_along(myvarsid)) {
    myvar <- myvarsid[i]  # Get the variable name
    
    # Check if the variable exists in the data frame
    if (myvar %in% names(Dat)) {
      if (is.character(Dat[[myvar]])) {
        # Convert to character if it's a character variable
        Dat[[myvar]] <- as.character(Dat[[myvar]])
      } else {
        # Convert to numeric if it's not a character variable
        Dat[[myvar]] <- as.numeric(Dat[[myvar]])
      }
      
      # Append the modified variable name to the new list
      new_list[[myvar]] <- Dat[[myvar]]
    } else {
      warning(paste("Variable", myvar, "not found in the data frame."))
    }
  }
  ...
  def <- stats::reshape(Dat, direction = "long", varying = list(mmidx = 7:26, 
                                                                ...
                                                                mm9 = 107:126, mm12 = 127:146, mm16 = 147:166), v.names = c("mmidx", 
                                                                                                                            "mm1", "mm2", "mm4", "mm8", "mm9", "mm12", "mm16"), timevar = "alt")
  ...
  def$exposure <- ifelse(def$upplim - def$lowlim + 1 < 0, 0, 
                         def$upplim - def$lowlim + 1)
  ...
  myvarsid <- c("ID", "v021", "v005", "v008", "v022", "rweight", 
                "mmidx", "mm1", "mm2", "mm4", "mm8", "mm9", "mm12", "mm16", group_var)
  ...
  rdef <- stats::reshape(def, direction = "long", varying = list(agegrp = 15:17, 
                                                                 expo = 18:20, death = 21:23), v.names = c("agegrp", "expo", 
                                                                                                           "death"), timevar = "alt")
