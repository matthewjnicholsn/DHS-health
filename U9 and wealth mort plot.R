...
# Create the bar chart with individual observations
ggplot(data, aes(x = Observation, y = R, fill = Class)) +
  geom_col() +
  labs(title = "Child Mortality Rates by Wealth Class",
       x = "Observation",
       y = "Child Mortality Rate (R)") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2") +
  scale_x_continuous(breaks = data$Observation, labels = data$Class)
